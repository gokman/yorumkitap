...
public class TimeLineActivityYedek extends ActivityBase implements OnClickListener{
    ...
    public void onCreate(Bundle savedInstanceState) {
			...
			try {
				...
				bookList=new SelectDataTask(TimeLineActivityYedek.this).execute(servicer).get();
				...
				for(int i=0;i<bookList.size();i++){
					...
					map.put(ApplicationConstants.TYPE_BOOK_OWNER, new DatabaseProcess().getUserName(tempBook1.get(ApplicationConstants.book_adderId).toString(),TimeLineActivityYedek.this));
				...
				//kitap bitir
					...
					commentList=new SelectDataTask(TimeLineActivityYedek.this).execute(servicerComment).get();
					...
					for(int j=0;j<commentList.size();j++){
				        ...
				        commentBook=new SelectDataTask(TimeLineActivityYedek.this).execute(serviceBook).get().get(0);
					...
					for(int z=0;z<commentBookList.size();z++){
						...
						commentBookTempMap.put(ApplicationConstants.TYPE_COMMENTEDBOOKOWNER, 
								new DatabaseProcess().getUserName(commentBookList.get(z).get(ApplicationConstants.book_adderId).toString(),TimeLineActivityYedek.this));
